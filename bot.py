...
def safe_symbol(sym: str) -> str:
    return sym.replace('.', '-')  # BRK.B -> BRK-B for Yahoo
...
def update_sp500_once_daily() -> List[str]:
    ...
    try:
        ...
        out = pd.DataFrame({'ticker': tickers, 'updated_at': today})
        out.to_csv(SP500_FILE, index=False)
        return tickers
    except Exception:
        if SP500_FILE.exists():
...
def get_run_universe() -> List[str]:
    all_ticks = update_sp500_once_daily()
    all_ticks = [t for t in all_ticks if t not in BLACKLIST]
    all_ticks = sorted(set(all_ticks).union(set(WHITELIST)))
    state = load_json(STATE_FILE, {'offset': 0})
    offset = int(state.get('offset', 0)) % max(len(all_ticks), 1)
    batch = (all_ticks + all_ticks)[offset:offset + MAX_TICKERS_PER_RUN]
    state['offset'] = (offset + MAX_TICKERS_PER_RUN) % max(len(all_ticks), 1)
...
def volume_profile_shelves(df: pd.DataFrame, bins: int = 24, top_n: int = 3) -> List[float]:
    ...
    if len(prices) < 5 or vols.sum() <= 0: return []
...
def get_option_chain(ticker: str, expiry: str):
    ...
    cp.write_text(json.dumps({'calls': calls.to_dict('records'), 'puts': puts.to_dict('records')}, default=str))
    return calls, puts
...
def save_option_volume_snapshot(ticker: str, expiry: str, calls: pd.DataFrame, puts: pd.DataFrame):
    ...
    for side, df in [('CALL', calls), ('PUT', puts)]:
        for _, r in df.iterrows():
            rows.append({'date': ts, 'ticker': ticker, 'expiry': expiry, 'side': side,
                         'strike': float(r['strike']), 'volume': float(r.get('volume', 0) or 0)})
    snap = pd.DataFrame(rows)
    if OPT_VOL_HISTORY.exists():
        old = pd.read_csv(OPT_VOL_HISTORY)
        snap = pd.concat([old, snap], ignore_index=True).drop_duplicates(['date','ticker','expiry','side','strike'], keep='last')
    snap.to_csv(OPT_VOL_HISTORY, index=False)

def avg_option_volume_20d(ticker: str, expiry: str, side: str, strike: float) -> float:
    if not OPT_VOL_HISTORY.exists(): return np.nan
    h = pd.read_csv(OPT_VOL_HISTORY)
    q = h[(h.ticker == ticker) & (h.expiry == expiry) & (h.side == side) & (h.strike == strike)]
...
def iv_rank_from_chains(chains: List[pd.DataFrame]) -> float:
    ...
    if len(iv) < 10: return np.nan
    cur = float(iv.median())
    lo, hi = float(iv.quantile(.05)), float(iv.quantile(.95))
    return np.nan if hi <= lo else 100 * (cur - lo) / (hi - lo)
...
def prescreen(ticker: str, tk: yf.Ticker, daily: pd.DataFrame, monthly: pd.DataFrame) -> Tuple[bool, str]:
    ...
    if adv <= 1_000_000 or not (15 <= S <= 600): return False, 'stock liquidity/price failed'
    if S <= float(ema(monthly['Close'], 50).iloc[-1]): return False, 'below monthly EMA50'
    if earnings_within_7d(tk): return False, 'earnings within 7 days'
    return True, 'ok'
...
def analyze_ticker(ticker: str) -> List[str]:
    ...
    fourh = tk.history(period='90d', interval='1h', auto_adjust=False)  # Yahoo has no true 4h; resample from 1h
    if not fourh.empty:
        fourh = fourh.resample('4h').agg({'Open':'first','High':'max','Low':'min','Close':'last','Volume':'sum'}).dropna()
    ok, why = prescreen(ticker, tk, daily, monthly)
    if not ok: return alerts
    S = float(daily['Close'].iloc[-1])
    hv = hv30(daily)
    ...
    for label, rng in {'30DTE': (28,32), '90DTE': (88,92)}.items():
        ...
        if (calls.get('volume', pd.Series(dtype=float)).fillna(0).sum() + puts.get('volume', pd.Series(dtype=float)).fillna(0).sum()) <= 0: continue
        save_option_volume_snapshot(ticker, expiry, calls, puts)
        ivr = iv_rank_from_chains([calls, puts])
        if not np.isfinite(ivr) or not (20 < ivr < 80): continue

        # Common technicals
        f = fourh.tail(60).copy()
        m = monthly.copy()
        f['ema9'], f['ema21'], f['ema50'], f['ema200'] = ema(f.Close,9), ema(f.Close,21), ema(f.Close,50), ema(f.Close,200)
        ...
        month_ema50, month_ema200 = float(ema(m.Close,50).iloc[-1]), float(ema(m.Close,200).iloc[-1])
        ...
        for side, chain in [('CALL', calls), ('PUT', puts)]:
            ...
            avg20 = np.nanmean([avg_option_volume_20d(ticker, expiry, side, float(k)) for k in sel['strike']])
            med_iv = float(sel['impliedVolatility'].replace([np.inf,-np.inf], np.nan).median())
            spread_pct = float(((sel['ask'] - sel['bid']) / ((sel['ask'] + sel['bid'])/2)).replace([np.inf,-np.inf], np.nan).median() * 100)
            ...
            triggers = []
            # 1 unusual flow; real 20d avg becomes reliable only after bot has stored 20 snapshots
            avg_ok = np.isfinite(avg20) and avg20 > 0 and vol > 3 * avg20
            if avg_ok and vol > oi and premium_total >= 100_000: triggers.append('Unusual options flow')
            # 2 gamma setup proxy: OI near spot
            near = chain[(chain['strike'] >= S*.97) & (chain['strike'] <= S*1.03)]
            if float(near['openInterest'].fillna(0).sum()) > 20_000: triggers.append('Gamma squeeze setup')
            # 3 IV edge
            if side == 'CALL' and np.isfinite(hv) and med_iv < hv - 0.05: triggers.append('IV edge: IV below HV30')
            if side == 'PUT' and np.isfinite(hv) and med_iv > hv + 0.05: triggers.append('IV edge: IV above HV30')
            # 4 4H confluence
            if len(f) >= 50 and fourh_shelves:
                shelf = min(fourh_shelves, key=lambda x: abs(x-S))
                if side == 'CALL' and S > f.ema50.iloc[-1] and S > shelf and f.ema9.iloc[-1] > f.ema21.iloc[-1]: triggers.append('4H bullish confluence')
                if side == 'PUT' and S < f.ema50.iloc[-1] and S < shelf and f.ema9.iloc[-1] < f.ema21.iloc[-1]: triggers.append('4H bearish confluence')
            # 5 monthly support/resistance shelf
            if monthly_shelves:
                if side == 'CALL' and any(0 <= (S-sh)/S <= .08 for sh in monthly_shelves): triggers.append('Monthly support nearby')
                if side == 'PUT' and any(0 <= (sh-S)/S <= .08 for sh in monthly_shelves): triggers.append('Monthly resistance nearby')
            ...
            if score >= ALERT_SCORE_MIN:
                ...
                lines = [
                    ...
                    f'ATM±2 strike focus: {", ".join(map(lambda x: str(round(x,2)), sel["strike"].tolist()))}',
...
def main():
    ...
    print(f'Running {len(batch)} tickers: {batch[:5]} ...')
    for ticker in batch:
        try:
            for alert in analyze_ticker(ticker):
                send_alert(alert)
        except Exception as e:
